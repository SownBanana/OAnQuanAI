    ...
    def __init__(self, game, args):
        ...
        with self.graph.as_default():
            ...
            h_conv2 = Relu(BatchNormalization(self.conv2d(h_conv1, args.num_channels, 'same'), axis=3,
                                              training=self.isTraining))  # batch_size  x board_x x board_y x num_channels
            h_conv3 = Relu(BatchNormalization(self.conv2d(h_conv2, args.num_channels, 'same'), axis=3,
                                              training=self.isTraining))  # batch_size  x (board_x-2) x (board_y-2) x num_channels
            h_conv4 = Relu(BatchNormalization(self.conv2d(h_conv3, args.num_channels, 'same'), axis=3,
                                              training=self.isTraining))  # batch_size  x (board_x-4) x (board_y-4) x num_channels
            h_conv4_flat = tf.reshape(h_conv1, [-1, args.num_channels * (self.board_x) * (self.board_y)])
            s_fc1 = Dropout(
            ...
            self.calculate_loss()
# đẻ nó đánh lần 2 xem khác k train với không train toàn 5/5 thế này :v ô toang à c thử bỏ lớp cnn xem
    def conv2d(self, x, out_channels, padding):
        return tf.layers.conv2d(x, out_channels, kernel_size=[3, 3], padding=padding, use_bias=False) # chẵn thì ppdding same làm sao đc, 3 vẫn đc đấy
...
class ResNet():
    def __init__(self, game, args):
        ...
        with self.graph.as_default():
            ...
            x_image = tf.reshape(self.input_boards,
                                 [-1, self.board_x, self.board_y, 1])  # batch_size  x board_x x board_y x 1
            x_image = tf.layers.conv2d(x_image, args.num_channels, kernel_size=(1, 1), strides=(1, 1), name='conv',
